...
def load_text_generator():
    return pipeline("text-generation", model="distilgpt2")  # استخدام نموذج أخف
...
if st.button("🚀 Start Analysis and Suggestions") and text_input.strip():
    if len(text_input.split()) > 200:
        st.warning("⚠️ Please enter a shorter text (max ~200 words) to avoid timeout or memory issues.")
    else:
        ...
        with tab2:
            ...
            prompt = "Suggest a catchy title for this content: " + text_input
            try:
                title = text_generator(prompt, max_length=25, num_return_sequences=1)[0]['generated_text']
                st.success(title.replace(prompt, "").strip())
            except:
                st.error("Failed to generate a title. Try reducing the input length.")
            ...
            try:
                ...
                st.error("Failed to paraphrase. Try reducing the input length.")
